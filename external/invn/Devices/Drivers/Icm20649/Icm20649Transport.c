...
struct inv_icm20649 * icm20649_instance = 0;

int inv_icm20649_read_reg(struct inv_icm20649 * s, uint8_t reg,	uint8_t * buf, uint32_t len)
{
	return inv_icm20649_serif_read_reg(&s->serif, reg, buf, len);
}

int inv_icm20649_write_reg(struct inv_icm20649 * s, uint8_t reg, const uint8_t * buf, uint32_t len)
{
	return inv_icm20649_serif_write_reg(&s->serif, reg, buf, len);
}

void inv_icm20649_sleep_100us(unsigned long nHowMany100MicroSecondsToSleep)  // time in 100 us
{
	inv_icm20649_sleep_us(nHowMany100MicroSecondsToSleep * 100);
}

long inv_icm20649_get_tick_count(void)
{
	return (long)inv_icm20649_get_time_us();
}

/* driver transport function */

void inv_icm20649_transport_init(struct inv_icm20649 * s)
{
	s->lastBank = 0x7E;
	s->lLastBankSelected = 0xFF;
}

static uint8_t check_reg_access_lp_disable(struct inv_icm20649 * s, unsigned short reg)
{
	switch(reg){
		...
		case REG_FIFO_R_W:       /** (BANK_0 | 0x72) */
			return inv_icm20649_ctrl_get_batch_mode_status(s);
...
static int inv_set_bank(struct inv_icm20649 * s, unsigned char bank)
{
	int result;
    ...
    result = inv_icm20649_read_reg(s, REG_BANK_SEL, &s->reg, 1);
	...
	s->reg &= 0xce;
	s->reg |= (bank << 4);
    result = inv_icm20649_write_reg(s, REG_BANK_SEL, &s->reg, 1);
...
*  @return     0 if successful.
*/
int inv_icm20649_write_mems_reg(struct inv_icm20649 * s, uint16_t reg, unsigned int length, const unsigned char *data)
{
    int result = 0;
	unsigned int bytesWrite = 0;
    unsigned char regOnly = (unsigned char)(reg & 0x7F);

    unsigned char power_state = inv_icm20649_get_chip_power_state(s);
    ...
    if(check_reg_access_lp_disable(s, reg))    // Check if register needs LP_EN to be disabled   
        result |= inv_icm20649_set_chip_power_state(s, CHIP_LP_ENABLE, 0);  //Disable LP_EN
	...
	while (bytesWrite<length) 
	{
        ...
        result |= inv_icm20649_write_reg(s, regOnly+bytesWrite,&data[bytesWrite], thisLen);
...
*  @return     0 if successful.
*/
int inv_icm20649_write_single_mems_reg(struct inv_icm20649 * s, uint16_t reg, const unsigned char data)
{
    int result = 0;
    unsigned char regOnly = (unsigned char)(reg & 0x7F);
    ...
    result |= inv_set_bank(s, reg >> 7);
    result |= inv_icm20649_write_reg(s, regOnly, &data, 1);
...
*  @return     0 if successful.
*/
int inv_icm20649_read_mems_reg(struct inv_icm20649 * s, uint16_t reg, unsigned int length, unsigned char *data)
{
	int result = 0;
	...
	while (bytesRead<length) 
	{
		...
		if(s->base_state.serial_interface == SERIAL_INTERFACE_SPI) {
			result |= inv_icm20649_read_reg(s, regOnly+bytesRead, &dat[bytesRead], thisLen);
		} else {
			result |= inv_icm20649_read_reg(s, regOnly+bytesRead, &data[bytesRead],thisLen);
		}
	...
	if(check_reg_access_lp_disable(s, reg))    // Check if register needs LP_EN to be enabled  
		result |= inv_icm20649_set_chip_power_state(s, CHIP_LP_ENABLE, 1);  //Enable LP_EN
...
*  @return     0 if successful.
*/
int inv_icm20649_read_mems(struct inv_icm20649 * s, unsigned short reg, unsigned int length, unsigned char *data)
{
	int result=0;
	...
	if(check_reg_access_lp_disable(s, reg))
		result |= inv_icm20649_set_chip_power_state(s, CHIP_LP_ENABLE, 0);
	...
	{
		result |= inv_icm20649_write_reg(s, REG_MEM_BANK_SEL, &lBankSelected, 1);
		if (result)
			return result;
	...
	while (bytesWritten < length) 
	{
		...
		result |= inv_icm20649_write_reg(s, REG_MEM_START_ADDR, &lStartAddrSelected, 1);
		if (result)
			return result;
		...
		/* Write data */
		if(s->base_state.serial_interface == SERIAL_INTERFACE_SPI) {
			result |= inv_icm20649_read_reg(s, REG_MEM_R_W, &dat[bytesWritten], thisLen);
		} else {
			result |= inv_icm20649_read_reg(s, REG_MEM_R_W, &data[bytesWritten], thisLen);
		}
		if (result)
			return result;
...
*  @return     0 if successful.
*/
int inv_icm20649_write_mems(struct inv_icm20649 * s, unsigned short reg, unsigned int length, const unsigned char *data)
{
    int result=0;
    unsigned int bytesWritten = 0;
    unsigned int thisLen;
    ...
    while (bytesWritten < length) 
    {
        ...
        /* Write data */ 
        result |= inv_icm20649_write_reg(s, REG_MEM_R_W, &data[bytesWritten], thisLen);
        if (result)
            return result;
...
*  @return     0 if successful.
*/
int inv_icm20649_write_single_mems_reg_core(struct inv_icm20649 * s, uint16_t reg, const uint8_t data)
{
    int result = 0;
    unsigned char regOnly = (unsigned char)(reg & 0x7F);

...
int inv_icm20649_firmware_load(struct inv_icm20649 * s, const unsigned char *data_start, unsigned short size_start, unsigned short load_addr)
{ 
    int write_size;
    int result;
    ...
    while (size > 0) {
        ...
        result = inv_icm20649_write_mems(s, memaddr, write_size, (unsigned char *)data);
        if (result)  
            return result;
        data += write_size;
        size -= write_size;
    ...
    while (size > 0) {
        ...
        result = inv_icm20649_read_mems(s, memaddr, write_size, data_cmp);
        if (result)

...
int inv_icm20649_mpu_set_FIFO_RST_Diamond(struct inv_icm20649 * s, unsigned char value)
{
	int result = 0;
	unsigned char reg;

	result |= inv_icm20649_read_mems_reg(s, REG_FIFO_RST, 1, &reg);
    
	reg &= 0xe0;
	reg |= value;
	result |= inv_icm20649_write_mems_reg(s, REG_FIFO_RST, 1, &reg);
    
	return result;
}

int inv_icm20649_identify_interrupt(struct inv_icm20649 * s, short *int_read)
{
    ...
    result = inv_icm20649_read_mems_reg(s, REG_INT_STATUS, 1, &int_status);
    if(int_read)
        *int_read = int_status;

    result = inv_icm20649_read_mems_reg(s, REG_DMP_INT_STATUS, 1, &int_status); // DMP_INT_STATUS
	if(int_read)
		*int_read |= (int_status << 8);
    
    /*if(wake_on_motion_enabled) {
        result = inv_icm20649_read_mems_reg(s, REG_INT_STATUS, 1, &int_status);//INT_STATUS
        if(result)
            return result;
        *int_read |= reg_data[1];
    }*/
     ...
     */
    //result = inv_icm20649_read_mems_reg(s, 0x1B, 1, &int_status);
...
**/
static int dmp_get_fifo_length(struct inv_icm20649 * s, uint_fast16_t * len )
{
	...
	result = inv_icm20649_read_mems_reg(s, REG_FIFO_COUNT_H, 2, fifoBuf);
	if (result) 
	{
...
*/
static int dmp_reset_fifo(struct inv_icm20649 * s)
{
	...
	{ 
		...
		result |= inv_icm20649_mpu_set_FIFO_RST_Diamond(s, 0x1f);
		result |= inv_icm20649_mpu_set_FIFO_RST_Diamond(s, 0x1e);
...
*  @return MPU_SUCCESS or non-zero error code
**/
static int dmp_read_fifo(struct inv_icm20649 * s, unsigned char *data, uint_fast16_t len)
{
	int result;
    ...
    while (bytesRead<len) 
    {
        ...
        result = inv_icm20649_read_mems_reg(s, REG_FIFO_R_W, thisLen, &data[bytesRead]);
        if (result)
		{
			dmp_reset_fifo(s);
			s->fifo_info.fifoError = -1;
...
**/
static uint_fast16_t dmp_get_fifo_all(struct inv_icm20649 * s, uint_fast16_t length, unsigned char *buffer, int *reset)
{
	int result;
...
*/
static int extract_sample_cnt(struct inv_icm20649 * s, int fifo_size, unsigned short * total_sample_cnt, unsigned short * sample_cnt_array)
{
...
int inv_icm20649_fifo_swmirror(struct inv_icm20649 * s, int *fifo_sw_size, unsigned short * total_sample_cnt, unsigned short * sample_cnt_array)
{
	int reset=0; 
...
int inv_icm20649_fifo_pop(struct inv_icm20649 * s, unsigned short *user_header, unsigned short *user_header2, int *fifo_sw_size)  
{
	...
	if (*fifo_sw_size > 3) {
		...
		fifo_ptr += inv_icm20649_inv_decode_one_ivory_fifo_packet(s, &fd, fifo_ptr);        
...
*/
int inv_icm20649_inv_decode_one_ivory_fifo_packet(struct inv_icm20649 * s, struct inv_fifo_decoded_t *fd, const unsigned char *fifo_ptr)
{
...
int inv_icm20649_dmp_get_accel(long acl[3])
{
    if(!acl) return -1;
...
int inv_icm20649_dmp_get_raw_gyro(short raw_gyro[3])
{
...
int inv_icm20649_dmp_get_gyro_bias(short gyro_bias[3])
{
...
int inv_icm20649_dmp_get_calibrated_gyro(signed long calibratedData[3], signed long raw[3], signed long bias[3])
{
...
int inv_icm20649_dmp_get_6quaternion(long quat[3])
{
    if(!quat) return -1;
...
*/
int inv_icm20649_get_gyro_accuracy(void)
{
